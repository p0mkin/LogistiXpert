...
exports.AuctionService = exports.redis = void 0;
...
exports.redis.defineCommand('placeBidAtomic', {
    numberOfKeys: 2,
    lua: PLACE_BID_LUA,
});
class AuctionService {
     ...
     */
    static async cacheAuction(auctionId) {
        ...
        if (auction.highestBidderId) {
            ...
            await exports.redis.zadd(`auction:${auction.id}:bids`, auction.currentBid.toString(), member);
        }
    }
    /**
     * Executes atomic bid via Redis Lua Script
     */
    static async placeBid(auctionId, bidderId, username, amount) {
        const key = `auction:${auctionId}`;
        const bidKey = `auction:${auctionId}:bids`;
        const now = Date.now();
        // Call the custom Lua script
        const result = await exports.redis.placeBidAtomic(key, bidKey, amount.toString(), bidderId, username, now.toString());
        ...
        await exports.redis.xadd('stream:auction_bids', '*', 'auctionId', auctionId, 'bidderId', bidderId, 'amount', amount.toString(), 'expiresAt', result[1]);
        return {
     ...
     */
    static async settleAuction(auctionId) {
        const key = `auction:${auctionId}`;
        ...
        await exports.redis.del(`auction:${auctionId}:bids`);
    }
    /**
