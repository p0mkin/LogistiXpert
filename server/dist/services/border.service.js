...
class BorderService {
     ...
     */
    static async calculateClearance(truckId, checkpoint) {
     ...
     */
    static async applyBustPenalties(truckId, penalties) {
     ...
     */
    static async applyClearanceSuccess(truckId) {
     ...
     */
    static async applyBribeAttempt(truckId, bribeAmount) {
        return await prisma.$transaction(async (tx) => {
            const truck = await tx.truck.findUnique({
                where: { id: truckId },
     ...
     */
    static async applyBorderRun(truckId) {
        return await prisma.$transaction(async (tx) => {
            const truck = await tx.truck.findUnique({
                where: { id: truckId },
                include: { owner: true, activeRoute: { include: { driver: true, contrabandJob: true } } },
