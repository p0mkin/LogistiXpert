...
// ================================================================
router.get('/fleet-value', auth_1.authenticateJWT, async (_req, res) => {
    try {
...
// ================================================================
router.get('/underworld-rep', auth_1.authenticateJWT, async (_req, res) => {
    try {
        const users = await prisma.user.findMany({
            select: {
                id: true,
...
// ================================================================
router.get('/mileage', auth_1.authenticateJWT, async (_req, res) => {
    try {
        const users = await prisma.user.findMany({
            select: {
                id: true,
...
// ================================================================
router.get('/heat-index', auth_1.authenticateJWT, async (_req, res) => {
    try {
        const users = await prisma.user.findMany({
            select: {
                id: true,
        ...
        function getWantedLevel(heat) {
            if (heat >= 90)
                return '☢️ EXTREME THREAT';
            if (heat >= 70)
                return '🔴 MOST WANTED';
            if (heat >= 50)
                return '🟠 HIGH ALERT';
...
// ================================================================
router.get('/auction-wins', auth_1.authenticateJWT, async (_req, res) => {
    try {
...
// ================================================================
router.get('/my-rank', auth_1.authenticateJWT, async (req, res) => {
    try {
        ...
        function computeFleetValue(user) {
