...
function calcRepairCost(
  engineHealth: number,
  tireWear: number,
  distanceFromHub: number // km estimate from nearest repair hub
): {
  engineCost: number;
  tireCost: number;
  towCost: number;
  ...
  if (engineHealth >= 70 && tireWear >= 40) severity = 'MINOR';
  else if (engineHealth >= 40 && tireWear >= 20) severity = 'MODERATE';
  else if (engineHealth >= 15 && tireWear >= 5) severity = 'SEVERE';
...
const CITY_TO_NEAREST_HUB_KM: Record<string, number> = {
  Tallinn: 0,
  Riga: 0,
  Vilnius: 0,
  Warsaw: 0,
...
// ================================================================
router.get('/estimate/:truckId', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
...
// ================================================================
router.post('/roadside-repair', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Log the event
      const repairDesc = [];
      if (repairEngine) repairDesc.push(`Engine restored to 100%`);
      if (repairTires) repairDesc.push(`Tires restored to 100%`);
...
// Body: { truckId, repairEngine: boolean, repairTires: boolean }
// ================================================================
router.post('/garage-repair', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
    const { truckId, repairEngine = true, repairTires = true } = req.body;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const repairDesc = [];
      if (repairEngine) repairDesc.push('Engine: 100%');
      if (repairTires) repairDesc.push('Tires: 100%');
...
// ================================================================
router.post('/release-impound/:truckId', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
    const { truckId } = req.params;
